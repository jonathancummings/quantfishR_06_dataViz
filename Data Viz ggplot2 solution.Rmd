---
title: "Data Viz ggplot2 - exercise"
...
##### Data layer
Let's start by refamilizing ourselves with our data.  First we need to get our data into R and join our data sets together.
...
Other useful functions for looking at a data frame are 'str()' and 'names()'.  To look at individual columns you may wish to use the 'class()', 'head()', 'unique()', and 'length()' functions, or some combination thereof.  e.g.,
...
Once we know what our data is, we may wish to further explore our data by creating some exploratoty plots. We likely aren't too concerned with the optional layers for this exploration, so let's focus on the required layers for now. The first required layer is the data, the table in which the data we wish to visualize is stored. This is the first argument to our vizualization function 'ggplot()'.  I.E.,  'ggplot(neus_data)'.  

If we only wish to look at the data from a single column we could use a bar or historgram geometry, e.g.,

```{r data explore, echo=TRUE}
# exploratory plots
ggplot(neus_data[1:100,],aes(COMNAME))+geom_bar()
ggplot(neus_data[1:10000,],aes(BIOMASS))+geom_histogram()
```
...
# exploratory plots
ggplot(neus_data[1:10000,],aes(BIOMASS))
```
We can see the structure or data will be visualized with, but note that there is no data visualized.

If we were creating an 2d plot, e.g., a plot of x by y, our aesthetics layer would require a different structure, with an x and a y component.
e.g, 
```{r data explore4, echo=TRUE}
# exploratory plots
ggplot(neus_data[1:10000,],aes(BIOMASS,ABUNDANCE))
```

This gives us a different structure for our 2d plot, but still no visualization.

##### Geometries layer
The third layer is our geometries layer. These are the visual elements that place our data into our aesthetic structure, such as bar charts, histograms, boxplots, etc. (Figure 3 &ast;, Geometries).
...
When you think about visualization the aspect you probably tend to think most about is what type of plot or visualization you want to create.  This is determined by the geometry layer, and where the standard, or baseline visualization options for ggplot2 live.  The RStudio ggplot2 cheat sheet contains a good summary of the geom options available. 
...
These three layers are combined using the 'ggplot()' function to create a visualization. We give ggplot the data we want to plot, the aesthetic structure into which to place the data, and the visualization to use for that data.  When the layers are combined correctly we get our visualization, e.g.,
```{r data explore5, echo=TRUE, warning=FALSE}
# exploratory plots
ggplot(neus_data[1:10000,],aes(BIOMASS,ABUNDANCE)) + geom_point()
```

If we try to visualize our data improperly - using a geometry without the correct structure to support it - we will get an error.

```{r data explore6, eval=FALSE, error=TRUE, include=FALSE}
# exploratory plots
ggplot(neus_data[1:10000,],aes(BIOMASS)) + geom_point()
ggplot(neus_data[1:10000,],aes(BIOMASS,ABUNDANCE)) + geom_histogram()
```
...
Here we correctly supply our data layer, a 2d structure, and a geometry that plots pairs of points in that structure:
```{r data viz exploration, echo=TRUE, warning=FALSE}
# plot two variables
ggplot(neus_data[1:1000,],aes(BIOMASS,ABUNDANCE)) +
  geom_point()
```
...
For more information on using gglot2, and a summary of the commands available for each layer see <https://ggplot2.tidyverse.org/reference/>.
